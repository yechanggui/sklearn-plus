            ...
            with self.sess.as_default():
                ...
                def train_step(x_batch, y_batch,save=False):
                    ...
                    _, step, summaries, loss, accuracy = self.sess.run(
                        [train_op, self.global_step, train_summary_op, self.model.loss, self.model.accuracy],
                        feed_dict)
                ...
                def dev_step(x_batch, y_batch, writer=None,save=False):
                    ...
                    step, summaries, loss, accuracy = self.sess.run(
                        [self.global_step, dev_summary_op, self.model.loss, self.model.accuracy],
                        feed_dict)
                    time_str = datetime.datetime.now().isoformat()
                    print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy))
                    if save:
                ...
                for batch in batches:
                    ...
                    if current_step % checkpoint_every == 0:
                        ...
                        print 'save model in batch loop'
                dev_step(x_dev, y_dev, writer=dev_summary_writer)
